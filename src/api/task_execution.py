...
@router.get("")
async def list_task_executions(session: db_dep, user_id: actual_user_id_dep):
...
@router.get("/all")
async def list_all_task_executions(session: db_dep, user_id: only_admin_dep):
...
async def retrieve_task_execution(task_execution_id: int, session: db_dep, user_id: actual_user_id_dep):
...
async def destroy_task_execution(task_execution_id: int, session: db_dep, user_id: actual_user_id_dep):
    task_execution = await session.get(TaskExecution, task_execution_id)
    if task_execution is None or task_execution.owner_id != user_id:
