...
async def create_manual_day(manual_day_schema: schemas.ManualDaySchema, session: db_dep,
    ...
    manual_day = await manual_day_crud.schema_owner_create(session, manual_day_schema, owner_id=user_id)
    await session.commit()
    return schemas.ManualDaySchema.model_validate(manual_day)
