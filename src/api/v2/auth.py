...
@router.post("/login")
async def login_user(form_data: Annotated[OAuth2PasswordRequestForm, Depends()], auth_schema: auth.AuthSchema,
                     session: db_dep):
    user = user_crud.get_by_name(form_data.username)
    if user is None or security.verify_password(form_data.password, user.hashed_password):
