...
@router.get("")
async def list_user_manual_manual_days(session: AsyncSession = Depends(get_db), user_id: int = Depends(get_user_id)) -> list[ManualDaySchema]:
...
@router.get("/all")
async def list_all_manual_days(session: AsyncSession = Depends(get_db), user_id: int = Depends(get_admin_id)) -> list[OwnerManualDaySchema]:
...
async def retrieve_user_manual_day(manual_day_id: int, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_user_id)) -> ManualDaySchema:
...
@router.get("/by_date")
async def retrieve_user_manual_day_by_date(manual_day_id: int, date_schema: DateSchema, session: AsyncSession = Depends(get_db),
...
@router.post("")
async def create_manual_day(manual_day_schema: CreateManualDaySchema, session: AsyncSession = Depends(get_db),
                            user_id: int = Depends(get_user_id)) -> ManualDaySchema:
...
@router.post("/bulk")
async def create_manual_days(manual_day_schemas: list[CreateManualDaySchema], session: AsyncSession = Depends(get_db),
...
async def update_user_manual_day(manual_day_id: int, session: AsyncSession = Depends(get_db),
...
async def destroy_user_manual_day(day_id: int, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_user_id)) -> dict:
