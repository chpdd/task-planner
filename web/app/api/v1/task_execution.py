...
@router.get("")
async def list_task_executions(session: AsyncSession = Depends(get_db), user_id: int = Depends(get_user_id)):
...
@router.get("/all")
async def list_all_task_executions(session: AsyncSession = Depends(get_db), user_id: int = Depends(get_admin_id)):
...
async def retrieve_task_execution(task_execution_id: int, session: AsyncSession = Depends(get_db),
...
async def destroy_task_execution(task_execution_id: int, session: AsyncSession = Depends(get_db), user_id: int = Depends(get_user_id)):
    task_execution = await session.get(TaskExecution, task_execution_id)
    if task_execution is None or task_execution.owner_id != user_id:
